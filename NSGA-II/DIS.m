function [opt,pop]=DIS(opt,pop)
% Function DIS: Deterministic infeasibility sorting
%
%***************************************************
...
%*************************************************************************
% 2. deterministic infeasibility sorting
%*************************************************************************
...
obj     = vertcat(pop.obj);
domMat  = calcInfeasDominationMatrix(nViol, violSum, obj,opt); % domination matrix for efficiency
...
function domMat = calcInfeasDominationMatrix(nViol, violSum, obj,opt)
...
for p = 1:N-1
    for q = p+1:N
        ...
        elseif(nViol(p) ~= 0 && nViol(q)==0)
            ...
            if strcmp(opt.sortingfun{1,1},'nds')==1 % non dominated sorting
                if(violSum(p) < violSum(q))
                    domMat(p, q) = 1;
                elseif(violSum(p) > violSum(q))
                    domMat(p, q) = -1;
                end
            elseif strcmp(opt.sortingfun{1,1},'dis')==1 % deterministic infeasiblity sorting
                pdomq = false;
                qdomp = false;
                for i = 1:numObj
                    if( obj(p, i) < obj(q, i) )         % objective function is minimization!

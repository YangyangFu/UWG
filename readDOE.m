    ...
    for j = 1:3
        for k = 1:16
            refDOE(i,j,k) = Building(hCeiling(j),...  % floorHeight
                ...
                'AIR',...                       % a/c type
                COP(j,k),...                    % cop
                SetCool(1,1)+273.15,...         % coolSetpointDay
                SetCool(1,1)+273.15,...         % coolSetpointNight
                SetHeat(1,1)+273.15,...         % heatSetpointDay
                SetHeat(1,1)+273.15,...         % heatSetpointNight
            ...
            if strcmp(TypeWall(j,k),'MassWall')
                ...
                wall = Element(0.3,0.92,thickness,layers,0,293,0);
...
%               roof = Element(alb,emis,thickness,[Concrete;Wood;Wood;Wood;Wood;Insulation],0,293,0);
            ...
            Schedule(i,j,k).Vent = Vent(j)/1000;     % m^3/m^2 per second
